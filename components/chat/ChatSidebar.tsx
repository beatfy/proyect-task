...
export default function ChatSidebar({
  ...
  useEffect(() => {
    if (projectId) {
      const stored = localStorage.getItem(`tasky-history-${projectId}`);
      if (stored) {
        try {
  ...
  useEffect(() => {
    if (projectId && messages.length > 0) {
      localStorage.setItem(
        `tasky-history-${projectId}`,
        JSON.stringify(messages.slice(-40)) // Keep last 40 messages
      );
    }
  }, [messages, projectId]);
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4">
              ...
              <div>
                <p className="font-medium text-sm">¡Hola! Soy Tasky 🤖</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Puedo ayudarte a gestionar tareas, proyectos y más.
                </p>
              </div>
        ...
        <div className="p-3 border-t border-border">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={1}
              className="flex-1 resize-none rounded-lg border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary max-h-32"
              style={{
                height: "auto",
                minHeight: "38px",
              }}
