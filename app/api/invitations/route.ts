    ...
    if (RESEND_API_KEY) {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); padding: 32px; border-radius: 12px 12px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">Has sido invitado a ${invitation.project?.name || "Beatfy"}</h1>
          </div>
          <div style="padding: 32px; background: white; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
            <p style="color: #374151; font-size: 16px; line-height: 1.6;">
              <strong>${inviter?.name || "Un usuario"}</strong> te ha invitado a unirte al proyecto <strong>${invitation.project?.name || ""}</strong> en Beatfy.
            </p>
      ...
      const resendRes = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          to: [email],
          subject: `${inviter?.name || "Un usuario"} te ha invitado a ${invitation.project?.name || "Beatfy"}`,
          html,
        }),
      });
